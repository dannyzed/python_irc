...
class PugHandler(handler.Handler):
    ...
    def __init__(self):
        ...
        self.funclist = {'join': self.fun_join, 'list': self.fun_list, 'leave': self.fun_leave, 'addplayer': self.fun_addplayer, 'delplayer': self.fun_delplayer, 'part': self.fun_leave, 'captain': self.fun_captain, 'pick': self.fun_pick, 'teams': self.fun_teams, 'listmaps': self.fun_listmaps, 'map': self.fun_map}
        self.maplist = {'as-sidisbad', 'as-ballistic', 'as-lavafort][', 'as-desertstorm', 'as-nosidmaps', 'as-theycrashthepugbot'}
    ...
    def fun_map(self,command):
        if self.mode == 'mappick':
            ...
            if h != -1:
                try:
                    h2 = self.maplist.index(mapname.lower())
                except ValueError:
                    h2 = -1
                if h2 != -1:
                    self.pickedmaps.append(self.maplist[h2])
                    self.mapspicked += 1
                    self.turnpick = (1+int((self.pickcount+3)/2)) % 2
                    if len(self.mapspicked) == self.maxmaps:
                        self.print_teams()
                        self.print_maps()
                        self.mode = 'playerjoin'
                        self.enter_player_join()
    ...
    def enter_map_pick(self):
        self.fun_listmaps([])
        self.pickedmaps = []

    def fun_listmaps(self,command):
        msg = ''
